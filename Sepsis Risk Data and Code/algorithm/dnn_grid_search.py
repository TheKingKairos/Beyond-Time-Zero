...
def make_loaders(X: torch.Tensor, y: torch.Tensor, batch_size: int, val_frac: float = 0.2):
    ds = TensorDataset(X, y)
    n_val = int(len(ds) * val_frac)
    n_train = len(ds) - n_val
    # Stratified-ish split: we’ll do an index-based split by shuffling — for strict stratified split, use sklearn
...
def grid_search(
    ...
    y,  # np.ndarray or torch.Tensor [N]
    layer_options: List[Tuple[int, ...]],
    activations: List[str],
    dropouts: List[float],
    use_batchnorm_options: List[bool],
    lrs: List[float],
    ...
    for i, (layers, act, do, use_bn, lr, wd, bs) in enumerate(combos, start=1):
        ...
        loaders = make_loaders(X, y, batch_size=bs, val_frac=0.2)
        cfg = TrainConfig(
            batch_size=bs,
            lr=lr,
            weight_decay=wd,
            ...
            best.update(
                {
                    ...
                    "model": model.state_dict(),  # CPU tensors
                    "metrics": metrics,
                }
            )
...
# Final metrics on full data: {'loss': 0.13069522391076144, 'auroc': 0.9898405274797208, 'acc': np.float64(0.9500497120841791)}
# {'layers': (256, 128, 64), 'activation': 'relu', 'dropout': 0.0, 'batchnorm': True, 'lr': 0.001, 'weight_decay': 1e-05, 'batch_size': 9192, 'epochs': 30, 'patience': 4, 'amp': True}
